    ...
    private void updatePathWithState(Path path, State state) {
        if (state.links != null) {
            ...
            for (int i = 0, count = state.points.length / 4; i < count; i++) {
                boolean exist = false;
                for (int j = 0; j < state.links.length; j++) if (state.links[j] == i) {
                    exist = true;
                    break;
                }
                if (exist)
                        ...
                        case XmlPullParser.END_TAG:
                            ...
                            switch(tagName) {
                                ...
                                case TAG_POINTS:
                                    ...
                                    for (int i = 0; i < state.points.length; i++) state.points[i] = Float.parseFloat(array.get(i));
                                    break;
                                case TAG_LINKS:
                                    state.links = new int[array.size()];
                                    for (int i = 0; i < state.links.length; i++) state.links[i] = Integer.parseInt(array.get(i));
                                    break;
                                case TAG_ITEM:
