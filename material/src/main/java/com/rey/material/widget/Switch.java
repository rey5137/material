...
public class Switch extends View implements Checkable, ThemeManager.OnThemeChangedListener {
    ...
    protected void applyStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (mTrackColors == null) {
            ...
            int[] colors = new int[] { ColorUtil.getColor(ThemeUtil.colorControlNormal(context, 0xFF000000), 0.5f), ColorUtil.getColor(ThemeUtil.colorControlActivated(context, 0xFF000000), 0.5f) };
            mTrackColors = new ColorStateList(states, colors);
        }
        if (mThumbColors == null) {
            int[][] states = new int[][] { new int[] { -android.R.attr.state_checked }, new int[] { android.R.attr.state_checked } };
            int[] colors = new int[] { 0xFAFAFA, ThemeUtil.colorControlActivated(context, 0xFF000000) };
